...
function SearchPage({pets, setPets, loading, setLoading, error, setError, hasMore, setHasMore, pageNumber, setPageNumber,
                      formData, setFormData, handleSearch}) {
  ...
  const filterPets = async (e, params) => {
    ...
    setLoading(true);
    try {
      const response = await requestData(1, formData.type, formData.location, params);
    ...
    <div className="result-page" ref={containerRef} onScroll={(e) => handleScroll(e)}>
      <div className="container" >
        <div className="search-bar-container">
          <Search containerRef={containerRef} formData={formData} setFormData={setFormData} handleSearch={handleSearchPet}/>
          {showFilterIcon &&
